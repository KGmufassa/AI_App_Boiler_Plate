# Stage 3 Dynamic Questionnaire Draft

## Purpose

This draft is a dynamic questionnaire for Stage 3 frontend planning.

It is designed to:

- minimize redundant questions
- derive as much as possible from Stage 1 and Stage 2 artifacts first
- ask only unresolved UI/UX and frontend experience questions
- produce enough clarity to complete:
...
1. Read Stage 1 and Stage 2 outputs first.
...
- likely need for dashboard, settings, auth, detail, review, and output surfaces
...
Ask only if settings/configuration is present in the PRD but its structure is unclear.
...
- What accessibility and motion baseline should Stage 3 assume?
  ...
  - high-accessibility / dense-workflow accommodations required
...
If a later question would only refine style but not change structure, stop and draft the plan.

---

## Suggested Output Schema

The questionnaire should populate an internal object like:
